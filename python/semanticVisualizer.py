...
Date: December 2015
Semantic feature extraction and clustering, generates to html file for visualizaiton
'''

# Import self generated files first to prevent recursive imports later
...
HTMLFILE = "./new.html"
# TODO: Add in Java files to repo and fix tag loc post computer switch
...
def shrinkDict(postThreshold = POST_THRESH):
	...
	for tagVector in tagList:
		try:
			# Printing sanity check for long data read in to ensure still running
...
# Globally declare tag dictionary
TAG_DICT = shrinkDict()
...
def generateDataPoints(dFile):
	...
	savetxt(RAWDATA, fData, fmt="%.9f", delimiter=' ')
	raw = open(RAWDATA)
